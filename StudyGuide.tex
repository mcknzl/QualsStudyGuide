...
The first principles derivation of thrust required for steady, level flight comes from force balance
$$T_R = D\quad L = W$$
Combining these equations gives
\CenteredBoxed{T_R = \frac{W}{L/D}}
From this we see minimum thrust required is proportional to (instantaneous weight) but \emph{independent of altitude}. Altitude indepence is implied by the ratio $L/D\big|_{max}$ being independent of altitude for this simple analysis.
...
\begin{wrapfigure}{r}{5cm}\centering\includegraphics[width=0.3\textwidth]{Graphics/simple_DSM.PNG}\end{wrapfigure}
...
\textbf{Definition:} A simple substance is one for which there is only one reversible work mode. A simple compressible substance is one where that reversible work mode is fluid compression ($pdV$ work).\\
...
\CenteredBoxed{dS = \frac{\delta Q}{T} + \delta S_{gen} \geq 0}
\normalsize
...
\begin{minipage}{0.48\textwidth}
	\centering
	\includegraphics[width=0.5\textwidth]{Graphics/fuel_ox_sphere.PNG}
\end{minipage}
...
Perfectly stirred reactors everywhere except for the (optional) burnout zone. This is based on a handful of papers on models of reacting flow CFD, where the PSR model provides a decent estimate of local conditions for large reactor networks. This type of system is typically very turbulent and is aided by recirculation zones for flame holding. It would be preferable to model the entire rich combustion zone as perfectly stirred because of these conditions, rather than as a premixed fluid simply flowing axially. More accurate models put equivalence ratio distributions over the network within the rich zone to emulate the stratified composition which is seen in fuel injection systems. The quick mix region is simply a dump of air, which will be a very turbulent process and we can simplify the analysis by assuming no reactions; just an instantaneous, perfect mixing of air with the rich products. The lean combustion zone could possibly be modeled as a PFR, unless there was some new fuel injected into the system or a distribution of equivalence ratios was desired, in which case a network of PSRs would be more appropriate. In the burnout zone we can assume all the hot stuff is just flowing through with no real crazy stuff happening, so a plug flow reactor model is fine. (The burnout zone isn't even totally necessary so we could just leave it out.) If a true reactor network is not used -- if the rich and lean combustion zones are modeled only by single reactors -- then the quick mix PSR is unnecessary. The effects of mixing can be achieved by merging the two streams at the inlet of the lean PSR, where the $Da=0$ assumption will take care of the mixing for us.

\Problem{19} Consider the reactor network created for Problem 18 operating with propane as the fuel at 10 atmospheres with each combustion reactor having a volume of 1 $m^3$. The network is operating at a steady state for a long time, with $\dot V = 10\ m^3/s$, $\phi=2.5$, and $T_{in} = 500\ K$, but experiences a sinusoidal fluctation in inlet temperature at time $t=0$, given below. The air at the inlet of the quick mix region is maintained at 1000 Kelvin. Enough air is added such that the equivalence ratio at the inlet of the lean zone would be 1.2 if no fuel burned in the rich zone. Sketch the mole fraction profiles of $H_2O$, $CO_2$, $CO$, and $NO$ at the exit of each combustion zone as a function of time. The temperature profile is given, in Kelvin, as
$$T_{in}(t) = 800 - 300\cos(t/10),\quad t\in[0,20\pi]$$
\vspace{-1cm}
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{Graphics/T_vs_t.png}
\end{figure}
...
\Problem{20}\\
\begin{minipage}{0.48\textwidth}
A rigid spherical vessel initially contains air at 300 K and 1 atm. A leak develops which allows hydrogen to enter the vesse at a constant molar rate. Is the vessel ever at risk of exploding? If so, what can be done to prevent the mixture from exploding? What if the temperature of the air were raised to 500 K?(Graph is from \emph{Explosion Limits of Hydrogen/Oxygen Mixtures at Initial Pressures up to 200 bar} by Schr{\"o}der, Emonts, Janssen, and Schulze. 2004.)
\end{minipage}
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=0.7\textwidth]{Graphics/spherical_H2-air.PNG}
\end{minipage}

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{Graphics/H2-air_explosions.PNG}
\end{figure}

What if the problem were inverted? That is, what would happen if a hydrogen storage tank developed a leak which allowed air into the vessel? 
